...
extension HomeView{
    ...
    private var slimRectSection: some View {
        ZStack {
            ...
            VStack {
                ...
                Text("Progress:").font(.title3).foregroundColor(.black).fontWeight(.medium).padding(.top, 15).padding(.bottom, 20)
                ...
                HStack(spacing: 1) {
                    ...
                    Text("Time left: ").font(.title2).fontWeight(.semibold).foregroundColor(.black.opacity(0.7))
                    Text(vm.showTimer()).font(.title2).fontWeight(.bold).foregroundColor(.black.opacity(0.7))
                }.frame(maxWidth: .infinity, alignment: .leading).padding(.bottom, 20).padding(.leading, 5)
                HorizontalSplitter().padding(.bottom)
                HStack {
                    Text("To repeat: \(vm.allUnansweredWords.keys.count)").font(.title2).fontWeight(.semibold).foregroundColor(.black.opacity(0.7))
    ...
    private var gameInformationInBigRect: some View {
        ZStack(alignment: .topLeading) {
            RoundedRectangle(cornerRadius: 25).fill(.white).shadow(color: .black.opacity(0.15), radius: 2)
            HStack {
                VStack(alignment: .leading, spacing: 1) {
                    Title(title: "Info of following game: ")
                    Text("Completed: \(vm.answeredOfAllAmount) / \(vm.selectedQuestionAmount)").font(.callout).foregroundColor(.black.opacity(0.5))
                    Text("Answered right: \(vm.answeredRight) / \(vm.selectedQuestionAmount)").font(.callout).foregroundColor(.black.opacity(0.5))
    ...
    private var bigRectangleSection: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 15) {
                ...
                Text("\(vm.selectedQuestionAmount) random words quiz").fontWeight(.medium).font(.callout).foregroundColor(.black.opacity(0.5))
                ...
                ZStack {
                    ...
                    if questionAmount == 0 {
                        ...
                        if vm.selectedQuestionAmount == questionAmount {
                            ...
                            Text("\(questionAmount)").foregroundColor(.black)
                        }
                    }
    ...
    private var letsGoButtonView: some View {
        ZStack {
            ...
            HStack(spacing: 15) {
                Title(title: "Let's Go!").font(.title)
                Image("snoopy")
                    .resizable()
                    .scaledToFit()
...
struct Title: View {
    let title: String
    var body: some View {
        Text(title).foregroundColor(.black.opacity(0.7)).fontWeight(.medium)
    }
}
struct NavBarButton: View {
    ...
    var body: some View {
        ZStack {
            Circle().foregroundColor(backgroundColor).shadow(color: Color.black.opacity(0.3), radius: 5, x: 1, y: 1)
